...
test = open("./food_test.txt","w",encoding='UTF-8')
p = requests.Session()
url=requests.get("https://www.dcard.cc/f/food")
...
for k in range(0,10):
    ...
    r = p.get("https://www.dcard.tw/_api/forums/food/posts",params=post_data, headers = { "Referer": "https://www.dcard.tw/", "User-Agent": "Mozilla/5.0" })
    data2 = json.loads(r.text)
    print(data2)
    for u in range(len(data2)):
        print(data2[u]["id"])
        print(data2[u]["title"])
        Temporary_url = "/f/food/p/"+ str(data2[u]["id"]) + "-" + str(data2[u]["title"].replace(" ","-"))
...
for i in a[2:]:
    ...
    for c in sel_jpg:
        ...
        img2 = pic.content
        pic_out = open("./food/"+str(q)+".png",'wb')
        pic_out.write(img2)
        pic_out.close()
test.close()
print("爬蟲結束")